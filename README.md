# GitServer — GitHub Actions で Python を無料実行

サーバーをレンタルせずに、GitHub Actions を使って Python スクリプトを
**無料** で実行・定期実行するためのリポジトリです。

- パブリックリポジトリ: 実行時間 **無制限で無料**
- プライベートリポジトリ: 毎月 **2,000 分まで無料**(無料プランの場合)

---

## ⏸ 現在の状態: 定期実行は【停止中】です

| 実行方法 | 状態 |
|---|---|
| 手動実行(ボタン) | ✅ **使えます** |
| 定期実行(cron) | ⏸ **停止中**(いつでも再開できます) |

自動では一切実行されません。動かしたいときだけ手動で実行してください。

### 定期実行を再開するには

`.github/workflows/run-python.yml` を開き、`schedule:` と `- cron:` の
先頭についている `# ` を外して保存(コミット)するだけです。

```yaml
on:
  schedule:
    - cron: "37 15 * * *"   # 毎日 00:37 JST
  workflow_dispatch:
```

再開後は、この変更をデフォルトブランチに入れた時点から有効になります。

---

## 仕組み

GitHub Actions は、GitHub が用意した仮想マシン(Ubuntu)上でコードを実行してくれる
サービスです。`.github/workflows/run-python.yml` に「いつ・何を実行するか」を定義
しておくと、GitHub が自動でスクリプトを実行します。

## ファイル構成

| ファイル | 役割 |
|---|---|
| `main.py` | 実行される Python スクリプト(ここに処理を書く) |
| `requirements.txt` | 必要なパッケージの一覧 |
| `.github/workflows/run-python.yml` | 実行スケジュールなどの設定 |
| `.github/workflows/auto-retry.yml` | 失敗時に自動で再実行する設定 |
| `output/latest_run.json` | 実行結果(自動でコミットされる) |

## 使い方

### 1. 手動で実行する

1. GitHub のリポジトリページで **Actions** タブを開く
2. 左側の **Run Python** を選択
3. **Run workflow** ボタンを押す

### 2. 定期実行のスケジュールを変える

`.github/workflows/run-python.yml` の `cron` を編集します。
**時刻は UTC(日本時間 −9 時間)** で指定する点に注意してください。

```yaml
schedule:
  - cron: "37 15 * * *"   # 毎日 00:37 JST
```

> **ポイント**: 分を `0`(毎時ちょうど)にすると世界中のジョブが集中し、
> 実行が 1〜2 時間遅れることがあります。`37` のように中途半端な分に
> ずらしておくと待ち時間が短くなります。

よく使う例(JST 換算):

| cron (UTC) | 実行タイミング (JST) |
|---|---|
| `37 15 * * *` | 毎日 0:37 |
| `37 0 * * *` | 毎日 9:37 |
| `37 * * * *` | 毎時 37 分 |
| `7,37 * * * *` | 30 分ごと |
| `37 15 * * 0` | 毎週月曜 0:37 |

> **注意**: 定期実行はデフォルトブランチ(main)にあるワークフローだけが動きます。
> また、GitHub の混雑状況により数分〜十数分遅れることがあります。
> 60 日間リポジトリに更新がないと定期実行は自動停止します(Actions タブから再有効化できます)。

### 3. 自分の処理を書く

`main.py` の「ここから実際の処理を書く」の部分を書き換えてください。
外部パッケージが必要な場合は `requirements.txt` に追加します。

```txt
requests
beautifulsoup4
```

### 4. API キーなどの秘密情報を使う

コードに直接書かず、GitHub の Secrets を使います。

1. リポジトリの **Settings → Secrets and variables → Actions** を開く
2. **New repository secret** で登録(例: `MY_API_KEY`)
3. `run-python.yml` のコメントアウトを外して環境変数として渡す

```yaml
env:
  MY_API_KEY: ${{ secrets.MY_API_KEY }}
```

4. Python からは `os.environ["MY_API_KEY"]` で読み取れます

## トラブルシューティング

### `The job was not acquired by Runner of type hosted even after multiple attempts`
### `Internal server error. Correlation ID: ...`

**コードの問題ではなく、GitHub 側の一時的な障害です。**
GitHub が用意する実行マシン(ランナー)を確保できずにタイムアウトしたことを意味します。
混雑時やGitHub の障害発生時にまれに起きます。

対応は自動化済みです:

- `auto-retry.yml` が失敗を検知して **2 分後に自動で再実行**します(最大 3 回まで)
- cron を毎時 00 分からずらして、混雑ピークを避けています

そのため基本的に**放置で問題ありません**。Actions タブで、失敗した実行に
再試行の記録(Attempt 2)が付いて緑になっていれば復旧しています。

手動で再実行したい場合は、失敗した実行を開いて右上の
**Re-run jobs → Re-run failed jobs** を押してください。

なお、GitHub 全体の障害かどうかは <https://www.githubstatus.com/> で確認できます。

### 実行が予定時刻より遅れる

GitHub の仕様です。定期実行は「その時刻以降、空きができ次第」実行される
ベストエフォート方式で、混雑時は 1 時間以上遅れることもあります。
分単位の正確さが必要な用途には向きません。

### 定期実行が止まった

- **60 日間リポジトリに更新がない**と自動停止します
  → Actions タブから手動で再有効化してください
  (このリポジトリは実行結果を毎回コミットするため、通常は止まりません)
- ワークフローが**デフォルトブランチ**にあるか確認してください。
  定期実行はデフォルトブランチのファイルだけが有効です

## 制限事項

- 1 回の実行は最長 6 時間まで(このリポジトリでは 30 分に設定)
- 常時起動のサーバー(Web サーバー等)としては使えません。
  「決まったタイミングで処理を実行する」用途向けです
- 定期実行の最短間隔は 5 分です
